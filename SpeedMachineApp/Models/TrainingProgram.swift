...
class SessionProgress: ObservableObject {
    ...
    /// Planned target-speed pool for this block (adaptive overrides take priority).
    /// Mirrors the precedence used by `currentTargetSpeed`.
    var plannedSpeedPool: [Int] {
        if let a = adaptiveSequence, !a.isEmpty { return a }
        if let s = block.sequence, !s.isEmpty { return s }
        if let p = block.protocol_, !p.isEmpty { return p.map { $0.speed } }
        if let t = block.targetSpeed { return [t] }
        return []
    }

    /// True when the block aims at more than one distinct target speed.
    /// Drives the labeled, finger-scrollable tach strip (Blocked / Make-in-row stay single-speed).
    var isMultiSpeed: Bool { Set(plannedSpeedPool).count > 1 }
    ...
    func recordPutt(actualSpeed: Float) {
        ...
        if let acceptRange = block.acceptRange {
            ...
            isInZone = SpeedMath.isInZone(actual: actualSpeed, min: acceptRange.min, max: acceptRange.max)
        } else {
            let t = TrainingProgramLoader.shared.getToleranceForSpeed(targetSpeed)
            tolerance = t
            isInZone = SpeedMath.isInZone(actual: actualSpeed, target: targetSpeed, tolerance: t)
        }
