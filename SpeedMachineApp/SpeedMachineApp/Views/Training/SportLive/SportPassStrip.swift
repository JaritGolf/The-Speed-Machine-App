...
struct SportPassStrip: View {
    ...
    let puttHistory: [PuttResult]   // full session history for tach bars
    let totalPutts: Int             // total putts in block (for bar width math)
    let target: Int                 // current target speed (for deviation calc)
    let tolerance: Float            // ±MPH
