...
struct ProgressDashboardView: View {
    ...
    var completedDays: [DayCompletionData] {
        dataService.getAllCompletedTracks()
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                StatBox(title: "Tracks Complete", value: "\(completedDays)/\(TrainingConstants.totalTracks)")
                StatBox(title: "Current Track", value: "\(currentTrack)")
...
struct SessionRowView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("Track \(session.dayNumber) • \(session.blockId ?? "Block")")
                    .font(.subheadline)
                    .fontWeight(.medium)
                    .foregroundColor(AppColors.primaryBlack)
