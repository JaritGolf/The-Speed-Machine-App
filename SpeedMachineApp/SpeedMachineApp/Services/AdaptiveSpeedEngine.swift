...
class AdaptiveSpeedEngine {
    ...
    func generateAdaptiveSequence(
        ...
        case .smartInterleaved:
            // Smart interleaved blocks apply mastery filtering internally via
            // generateSmartInterleavedResult — no extra filtering needed here.
            return generateSmartInterleavedResult(for: block, track: track)?.sequence
        case .full:
            // Layer C: filter pool through per-speed mastery lock (Phase 2).
            // Locked speeds are excluded; their rep slots are filled by the highest
            // unlocked speed so the block still runs to its full putt count.
    ...
    /// Filters a speed pool through `MasteryService.isSpeedUnlockedForAdaptive`.
    /// Falls back to the full pool when every speed is locked (e.g., brand-new user).
    ...
    func generateSmartInterleavedResult(
        ...
        // Layer C: apply mastery lock so unlocked speeds only appear once the
        // prerequisite speed is at Tier 1+. Falls back to full pool for new users.
    ...
    /// Weakest speeds in pool appear most; builds contextual interference from early tracks.
