...
struct GateTest: Codable, Identifiable {
    ...
    /// "soft" or "hard" — matches GateForce enum; stored in JSON for documentation.
    /// Code-side enforcement uses MasteryService.gateForce(forTrack:).
    let gateForce: String?

    enum CodingKeys: String, CodingKey {
        case gateId = "id"
        case name, track, unlocksZone, unlocksSpeeds
        case protocol_ = "protocol"
        case totalPutts, passRequirements, gateForce
    }
}
...
/// Pass requirements for a training block or gate test.
/// Phase 4+ gate tests use the four-criterion fields (minOverallInZone etc.).
/// Legacy blocks (and any block without explicit criteria) may use zoneAccuracy.
struct PassRequirements: Codable {
    // Phase 4 gate test four-criterion schema
    let minOverallInZone:  Int?
    let minPerSpeedInZone: Int?
    let avgDeviationCapMph: Float?
    let maxSingleMissMph:  Float?
    // Legacy Phase 1 schema — kept for backward compatibility
...
struct TrainingBlock: Codable, Identifiable {
    ...
    // MARK: - Phase 1 Skill Gating fields (read from JSON; enforcement begins Phase 3)
    /// Per-block pass threshold override (0.0–1.0). nil = use phase floor.
    let blockPassThreshold: Float?
    /// true = completely skip gating for this block (free practice, warmups, recovery, etc.)
    let skipGating: Bool?
...
class TrainingProgramLoader: ObservableObject {
    ...
    private init() {
        loadFromBundle()
        // Asynchronously check for a remote update — does not block app launch
        Task {
            await NetworkService.shared.fetchProgramIfNeeded()
        }
    }

    // Called by NetworkService. Must be called via await MainActor.run { } from background threads.
