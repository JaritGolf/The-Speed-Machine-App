...
class TrainingViewModel: ObservableObject {
    ...
    func completeBlock() {
        ...
        if block.type != .gateTest {
            ...
            if !evaluation.passed && evaluation.gateForce == .soft {
    ...
    func repeatBlock() {
        ...
        // Hard gate: auto-halve reps on 3rd failed attempt (failedAttemptCount ≥ 2 = this is the 3rd)
    ...
    private func advanceToNextBlockOrComplete(track: TrainingTrack, block: TrainingBlock, session: SessionProgress) {
        ...
        if let currentBlockIndex = track.blocks.firstIndex(where: { $0.blockId == block.blockId }) {
            if currentBlockIndex < track.blocks.count - 1 {
                ...
                nextBlockForTransition = nextBlock
    ...
    func evaluateGateTest() {
        ...
        let result = MasteryService.shared.evaluateGateTest(
            session: session,
            block: block,
            gateId: gateId,
        ...
        if passed {
            ...
            if let track = selectedTrack {
                ...
                if completedBlockCount >= track.blocks.count - 1 { // -1 because current block isn't recorded yet
                    dataService.markTrackComplete(
                        trackNumber: track.number,
                        accuracy: session.zoneAccuracy,
                        totalPutts: session.currentPutt,
...
struct GateTestResult {
    ...
    let perSpeedAccuracy: [Int: Float]?
    let avgAbsDeviation: Float?
    let maxDeviation: Float?
    let failureReasons: [GateFailureReason]
