...
struct BlockEvaluation {
    ...
    /// Prior non-override failures for this block+track. Hard gate override unlocks at ≥ 3.
...
class MasteryService {
    ...
    // MARK: - Block Evaluation (Phase 1: compute only, no enforcement)

    /// Evaluates a block against its threshold. Returns an evaluation struct.
    /// In Phase 1 this is telemetry-only — the result is logged but not acted on.
    ...
    func evaluateGateTest(
        ...
        let speedsFailing = protocolSpeeds.filter { speedInZone[$0, default: 0] < minPerSpeed }
    ...
    /// Resets all SpeedProfileData and replays PuttRecordData to produce accurate
    /// mastery tiers for users who advanced before the gating system shipped.
    func recomputeFromHistory() {
        ...
        for p in profiles { profileMap[Int(p.targetSpeed)] = p }
        ...
        print("✅ MasteryService.recomputeFromHistory() complete — \(allPutts.count) putts replayed")
    }
    ...
    /// below `speed` within the given pool. The lowest speed in any pool is always
    /// unlocked — there is no prerequisite for the entry point.
